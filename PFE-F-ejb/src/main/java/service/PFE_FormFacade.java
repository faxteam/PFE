...
public class PFE_FormFacade extends AbstractFacade<PFE_Form> implements PFE_FormFacadeRemote {
	...
	public void SetCategory_PfeForm( Long cat_id, Long pfe_form_id ) {
		Categorys c =em.find(Categorys.class, cat_id);
	...
	public List<PFE_Form> FindProfForms(long l) {
		List<PFE_Form> forms = new ArrayList<>();
        
        try{
             forms = 
                     em.
                    createQuery("SELECT P FROM PFE_Form P where p.supervisor.employee_id = :id", PFE_Form.class)
                    .setParameter("id", l).getResultList();
            return (List<PFE_Form>) forms;
        }catch(NoResultException ex){
            return new ArrayList<PFE_Form>() 
			;
        }
	}
